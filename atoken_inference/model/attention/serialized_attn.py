...
def calc_serialization(
    ...
    serialize_coords += torch.tensor(shift_window, dtype=torch.int32, device=tensor.device).reshape(
        1, 3
    )
    ...
    for bi, s in enumerate(tensor.layout):
        ...
        if num_windows == 1:
            ...
            split = [
                math.floor(i * valid_window_size + shift_sequence) for i in range(num_windows + 1)
            ]
            ...
            for i in range(num_windows):
                ...
                fwd_indices.append(
                    to_ordered[
                        torch.arange(padded_start, padded_end, device=tensor.device) % num_points
                    ]
                )
